...
def initialize_tracks(grouped_measurements):
    tracks = {}
    for group_idx, group in enumerate(grouped_measurements):
        tracks[group_idx] = group
    return tracks
...
def plot_track_data(updated_states):
    ...
    r, az, el = cart2sph2(filtered_values_csv[:, 1], filtered_values_csv[:, 2], filtered_values_csv[:, 3])
    number = 1000
...
def main():
    ...
    track_database = {}  # Dictionary to store tracks and their measurements
    ...
    for group_idx, track_group in enumerate(grouped_measurements):
        ...
        for i, (x, y, z, mt) in enumerate(track_group):
            if i == 0:
                ...
                # Break out of the loop after the second initialization
                break

        # Store measurements in the track database
        track_database[group_idx] = track_group
        print("ccccccccccc:",track_database)
        # Proceed to further steps
        clusters = generate_clusters(track_database, track_group, cov_inv)
        hypotheses = generate_hypotheses(clusters)
        probabilities = calculate_joint_probabilities(hypotheses, track_database, clusters, cov_inv)
